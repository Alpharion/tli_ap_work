...
# Global URL cache for NATO Admiralty rating
...
def research_regulatory_status(oem_name: str, product_name: str, provider: str) -> list:
    """For a given OEM, search each regulatory body for approval/registration status."""
    bodies_str = ", ".join(f"{b['id']} ({b['country']})" for b in REGULATORY_BODIES)
    evidence = get_evidence(
        f"{oem_name} {product_name} FDA EMA TGA HSA PMDA MHRA NMPA regulatory approval registration",
        provider
    )
    evidence_note = (
    ...
    prompt = f"""You are a pharmaceutical regulatory affairs researcher.
Company: {oem_name}
Product: {product_name}

{evidence_note}

Determine whether "{oem_name}" holds or has sought regulatory approval/registration for "{product_name}"
(or its active pharmaceutical ingredients / drug substance) from each of the following bodies:
{bodies_str}

Return ONLY a valid JSON array (no markdown). One element per regulatory body:
{{
  "body": "FDA",
  "full_name": "Food and Drug Administration",
  "country": "USA",
  "status": "approved | registered | pending | not found | unknown",
  "details": "brief description e.g. NDA/ANDA number, marketing authorisation number, or why not found",
  "confidence": "high | medium | low"
}}

Use "not found" when you have strong evidence the company has NOT registered there.
Use "unknown" when you simply lack enough information.
Always return all {len(REGULATORY_BODIES)} bodies in the array."""

    try:
        raw = call_ai(prompt, provider, max_tokens=1200)
        parsed = safe_parse_json(raw)
        if isinstance(parsed, list):
            return parsed
    except Exception as e:
        print(f"[regulatory] parse error for {oem_name}: {e}")
    return [{"body": b["id"], "full_name": b["full"], "country": b["country"],
             "status": "unknown", "details": "Research failed", "confidence": "low"}
    ...
    for oem in oem_list:
        ...
        if should_verify:
            ...
            if not verification.get("verified"):
                ...
                oem["flagged"] = True
    '''
    # ── Market Share Research (IDC/Gartner/Statista) ─────────────────────────
...
Rank 1=largest. Only include companies from the known OEMs list."""
    ...
    if is_pharma and oem_list:
        push("status", message="💊 Pharmaceutical product detected — researching regulatory approvals…")
        pname = product_info.get("product_name", product_input)
        for oem in oem_list:
            oem_name = oem.get("company_name", "")
            if not oem_name or oem_name.lower() == "unknown":
                continue
            push("status", message=f"  🔍 Checking regulatory status for {oem_name}…")
            reg_results = research_regulatory_status(oem_name, pname, provider)
            regulatory_by_oem.append({
                "company_name": oem_name,
                "country": oem.get("country", ""),
                "regulatory_bodies": reg_results
            })
...
def export_docx():
    ...
    try:
        ...
        if oems:
            ...
            # Explicitly match column lengths to prevent index out of bounds
            ...
            # Write Data Rows natively looping over data array
            for ri, oem in enumerate(oems, 1):
                row = otbl.rows[ri]
                vals = [
                    str(oem.get('market_rank', '') or '—'),
                    oem.get('company_name', '—'),
                    oem.get('country', '—'),
                    oem.get('role', '—'),
                    oem.get('market_share', '—'),
                    oem.get('market_share_pct', '—'),
                    oem.get('market_share_source', '—'),
                    oem.get('notes', '—')
                ]
        ...
        # ── Regulatory Bodies (pharmaceutical) ──
        regulatory = data.get('regulatory', [])
        if regulatory:
            doc.add_heading('5. Regulatory Approval Status', level=1)
            intro = doc.add_paragraph(
                'The table below shows the known regulatory approval or registration status '
                'of each OEM for the identified pharmaceutical product across seven major regulatory bodies.'
            )
            intro.runs[0].font.size = Pt(10)
            doc.add_paragraph()

            reg_body_ids = ["FDA", "EMA", "TGA", "HSA", "PMDA", "MHRA", "NMPA"]
            STATUS_COLORS = {
                "approved":   "1A6B3C",
                "registered": "1A6B3C",
                "pending":    "8B6914",
            ...
            for oem_reg in regulatory:
                oem_heading = doc.add_heading(oem_reg.get('company_name', ''), level=2)
                oem_heading.runs[0].font.color.rgb = RGBColor(0x1A, 0x23, 0x7E)
                country_p = doc.add_paragraph(f"📍 {oem_reg.get('country', '—')}")
                country_p.runs[0].font.size = Pt(9)
                country_p.runs[0].font.color.rgb = RGBColor(0x66, 0x70, 0x94)

                bodies = {b.get('body', ''): b for b in oem_reg.get('regulatory_bodies', [])}
                reg_headers = ['Regulatory Body', 'Country/Region', 'Status', 'Details', 'Confidence']
                reg_widths  = [Inches(1.0), Inches(1.0), Inches(1.0), Inches(3.0), Inches(0.7)]

                rtbl = doc.add_table(rows=len(reg_body_ids) + 1, cols=5)
                ...
                for ci, (hdr, w) in enumerate(zip(reg_headers, reg_widths)):
                    hrow.cells[ci].width = w
                    add_cell_text(hrow.cells[ci], hdr, bold=True, size=9, color=RGBColor(0xFF, 0xFF, 0xFF))
                    set_cell_bg(hrow.cells[ci], '1A237E')

                body_meta = {b["id"]: b for b in REGULATORY_BODIES}
                for ri, bid in enumerate(reg_body_ids, 1):
                    entry = bodies.get(bid, {})
                    status = entry.get('status', 'unknown').lower()
                    bg = 'F8F9FA' if ri % 2 == 0 else 'FFFFFF'
                    drow = rtbl.rows[ri]
                    vals = [
                        bid,
                        body_meta.get(bid, {}).get('country', '—'),
                        status.title(),
                        entry.get('details', '—'),
                    ...
                    for ci, (val, w) in enumerate(zip(vals, reg_widths)):
                        ...
                        status_color = None
                        if ci == 2:
                            hex_c = STATUS_COLORS.get(status, '555555')
                            r_c = int(hex_c[0:2], 16)
                            g_c = int(hex_c[2:4], 16)
                            b_c = int(hex_c[4:6], 16)
                            status_color = RGBColor(r_c, g_c, b_c)
                        add_cell_text(drow.cells[ci], val, bold=(ci == 0), size=9,
                                      color=status_color if ci == 2 else None)
                        set_cell_bg(drow.cells[ci], bg)
                doc.add_paragraph()
...
def export_xlsx():
    try:
        ...
        product_name = data.get("product", {}).get("product_name", "supply_chain")
        result = [{
            "product_input": product_name,
            "supply_chain": data,
            "provider_results": {},
            "sheet_prefix": re.sub(r'[\\/*?\[\]:]', '', product_name).strip()[:26] or "Sheet1"
        }]

        depth = max((int(k.replace("tier_", "")) for k in data.get("tiers", {}).keys()), default=1)
        wb_bytes = build_bulk_workbook(result, data.get("provider", "unknown"), depth)

        safe_name = re.sub(r'[^a-zA-Z0-9_-]', '_', product_name)[:40]
        return Response(
            wb_bytes,
            mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            headers={"Content-Disposition": f'attachment; filename="{safe_name}_supply_chain.xlsx"'}
        )
    except Exception as e:
        return jsonify({"error": str(e)}), 500
